...
// 【注释】
// PassThrough 是 Node.js 中的一个流类，用于将输入流的数据直接传递到输出流，不进行任何处理。
...
const ChartSpaceXform = require('./xform/chart/chart-space-xform');
...
class XLSX {
  ...
  /**
   * 从文件系统读取Excel文件
   * @param {string} filename - 要读取的Excel文件路径
   * @param {object} options - 读取选项
   * @returns {Promise<Workbook>} 返回一个Promise,解析为工作簿对象
   *
   * 方法执行步骤:
   * 1. 首先检查文件是否存在
   * 2. 创建文件读取流
  ...
  async readFile(filename, options) {
    // 检查文件是否存在,使用utils.fs.exists进行异步检查
    ...
    try {
      // 调用this.read方法读取流中的Excel内容
      // this.read会解析Excel文件并创建workbook对象
  ...
  /**
   * 协调(reconcile)模型数据,处理各组件之间的关系和引用
   * 主要处理工作簿、工作表、图形、表格等组件之间的关联
   * @param {Object} model - Excel文件的数据模型
  ...
  // 以 _process 开头的函数主要用于处理 Excel 文件中的不同组件。
  // 它们的作用是解析特定类型的条目（如工作表、注释、表格等），并将解析后的数据存储到模型中。

  async _processWorksheetEntry(stream, model, sheetNo, options, path) {
    // 创建一个 WorksheetXform 实例，用于解析工作表数据
    const xform = new WorksheetXform(options);
    // 使用 parseStream 方法解析输入流，得到工作表对象
    const worksheet = await xform.parseStream(stream);
    // 将工作表的编号 (sheetNo) 赋值给工作表对象
    worksheet.sheetNo = sheetNo;
    // 将工作表对象存储到模型的 worksheetHash 中，以路径为键
    model.worksheetHash[path] = worksheet;
    // 将工作表对象添加到模型的 worksheets 数组中
  ...
  // TODO 如果读取图表文件，会大大增加解析时间，所以应该增加一个选项，选择是否需要图表。
  async _processChartEntry(stream, model, name) {
    const xform = new ChartSpaceXform();
    const chart = await xform.parseStream(stream);
    model.charts[name] = chart;
  }
        ...
        switch (entryName) {
          ...
          default: {
            ...
            if (!ignoreCharts) {
              ...
              if (match) {
                await this._processChartEntry(stream, model, match[1]);
                break;
              }
            }
          }
